...
如果在 OpenClaw 中使用，建议固定在同一个 agent 会话里调用 `WeChat-2__*` 工具，避免混用不同前缀实例。
...
如果你需要严格指定调用范围，可补一句：`只使用 WeChat-2__ 前缀工具`。

## Daily Workflow / 每日微信工作流（可直接复制）
...
/new wechat-mcp
/tools verbose
如果没有 WeChat-2__get_recent_sessions，就提示我重开会话
```
...
```text
只使用 WeChat-2__ 工具。
先获取新消息，再按「重要联系人 / 工作相关 / 待办事项 / 可忽略资讯」四类汇总。
过滤公众号营销和系统通知（但保留支付、订单、日程类通知）。
每类给 3-5 条要点，最后输出「我现在要做的 3 件事（含优先级和原因）」。
```

快速跟进模板（指定会话）：

```text
只使用 WeChat-2__ 工具，查看“某某群”最近 50 条消息。
输出：时间线摘要、关键结论、需要我回复的 3 条内容（附建议回复草稿）。
```

关键告警规则模板（付款/紧急/客户/会议）：

```text
只使用 WeChat-2__ 工具，先获取新消息并做告警检测。
按以下规则输出「高优先级告警」：
1) 付款类关键词：已支付、转账、退款、发票、尾款、催款；
2) 紧急类关键词：紧急、马上、尽快、今天内、截止；
3) 客户类关键词：客户、合同、报价、方案、验收；
4) 会议类关键词：开会、会议、日程、改期、地点、腾讯会议。
