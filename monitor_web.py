...
try:
    ...
    print("[AutoReply] 系统导入成功")
except ImportError as e:
    AUTO_REPLY_ENABLED = False
    print(f"[AutoReply] 导入失败: {e}")
    print("[AutoReply] 将在只读模式下运行")

# 全局自动回复实例
auto_reply_system = None
_cfg = load_config()
DB_DIR = _cfg["db_dir"]
        ...
        for ts, base, mc in hidden_msgs:
            ...
            with messages_lock:
                ...
                if len(messages_log) > MAX_LOG:
                    ...
                    # 自动回复处理
            if AUTO_REPLY_ENABLED and auto_reply_system:
                try:
                    # 准备消息数据
                    message_data = {
                        "username": username,
                    ...
                    if result:
                        ...
                        if reply_text:
                            print(f"[AutoReply] 生成回复: {reply_text}")
                except Exception as e:
                    print(f"[AutoReply] 处理消息异常: {e}")
                ...
                if AUTO_REPLY_ENABLED and auto_reply_system:
                    try:
                        ...
                        message_data = {
                            "username": msg['chat'],
                            "content": msg['content'],
                            "sender": sender or msg['chat'],
                            "is_group": '@chatroom' in msg['chat'],
                        ...
                        if result:
                            reply_text = result.get("reply", "")
                            if reply_text:
                                print(f"[AutoReply] 生成回复: {reply_text}", flush=True)
                    except Exception as e:
                        print(f"[AutoReply] 处理消息异常: {e}", flush=True)
            except Exception:
...
def main():
    ...
    if AUTO_REPLY_ENABLED:
        ...
        print(f"[AutoReply] 当前模式: {auto_reply_system.mode}")
    else:
        print("[AutoReply] 系统未启用，只读模式")
